...
class SP500Parser:
    ...
    def download_sp500_data(self, start_date: datetime, end_date: date) -> pd.DataFrame:
        """
        Downloads historical data for S&P 500 companies within the specified date range.
        ...
        df = data.stack().reset_index().rename(index=str,
                                               columns={"level_1": "Symbol"}).sort_values(['Symbol', 'Date']).set_index('Date')
        self.data = df
        return self.data
