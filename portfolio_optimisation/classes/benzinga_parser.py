...
class BenzingaNewsParser:
    ...
    def get_ticker_news(
            self, ticker: str, page: int, date_from: str
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if news_df is not None:
                    self.main_df = pd.concat(
                        [self.main_df, news_df], ignore_index=True
                    )
                    self.main_df = self.main_df.drop_duplicates(subset=["id"])

        self.main_df["stocks"] = self.main_df["stocks"].apply(
            lambda x: [entry["name"] for entry in x]
        )
        self.main_df = self.main_df.explode("stocks")
        self.main_df["updated"] = pd.to_datetime(
            pd.to_datetime(self.main_df["updated"]).dt.strftime('%Y-%m-%d')
        )
    ...
    def get_news(
            self, ticker, page, date_from, date_to, display_output="full"
                                                                            ):
        if not isinstance(ticker, str):
            ticker = ",".join(ticker)
